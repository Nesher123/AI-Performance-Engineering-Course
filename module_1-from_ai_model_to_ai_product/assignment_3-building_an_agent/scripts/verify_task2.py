...
CASES: list[MultiTurnCase] = [
    MultiTurnCase(
        ...
        # real assertion.
        notes=(
            "Pass = (a) both human messages persisted under thread_id 'demo' "
            "across the simulated restart, (b) turn 2 produced an AI answer."
        ),
    ),
    MultiTurnCase(
...
def run_case(case: MultiTurnCase, ckpt_path: Path) -> CaseResult:
    ...
    for turn in case.turns:
        ...
        config: RunnableConfig = {"configurable": {"thread_id": turn.session}}
        initial: GraphState = {
    ...
    last_session = case.turns[-1].session
    cp = get_checkpointer(ckpt_path)
    graph = build_graph(checkpointer=cp)
    final_state = graph.get_state({"configurable": {"thread_id": last_session}}).values
