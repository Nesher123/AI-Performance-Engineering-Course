...
class CommAndThroughputCallback(TrainerCallback):
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            ...
            if self.rank == 0:
                print(
                    f"[Comm] Registered timed all-reduce comm hook (world_size={self.world_size})."
                )
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        print(
            f"[Comm]   trainable parameters:                    {self.num_trainable_params:,}"
        )
        print(
            f"[Comm]   theoretical grad payload / step (fp32):   {grad_payload_bytes:,} bytes"
        )
        print(
            f"[Comm]   theoretical ring all-reduce bytes/GPU/step:{ring_bytes_per_gpu:,.0f} bytes"
        )
        print(
            f"[Comm]   measured total comm time (whole run):     {_comm_stats['total_time_s']:.4f} s"
        )
        print(
            f"[Comm]   measured total bytes communicated:        {_comm_stats['total_bytes']:,} bytes"
        )
        print(
            f"[Comm]   measured avg comm time / all-reduce call: {_comm_stats['total_time_s'] / num_allreduces:.6f} s"
        )
        print(
            f"[Comm]   measured avg comm time / optimizer step:  {_comm_stats['total_time_s'] / total_steps:.6f} s"
        )
        print("=" * 70)
...
def main():
    ...
    if rank == 0:
        print("[NCCL] Distributed training initialised")
    ...
    if rank == 0:
        ...
        ring_bytes_per_gpu = (
            2 * (world_size - 1) / world_size * grad_payload_bytes
            if world_size > 1
            else 0
        )
    ...
    eval_dataset = load_dataset(
        "Salesforce/wikitext", "wikitext-2-v1", split="validation"
    )
    ...
    def group_texts(examples):
        ...
        return {
            key: [
                values[i : i + block_size] for i in range(0, total_length, block_size)
            ]
