...
def _parse_verdict(text: str) -> tuple[bool, str]:
    """Defensively parse a {"ok": bool, "issue": str} object from an LLM reply.

    The model may wrap the JSON in prose or fences, so we grab the first {...}
    block. If parsing fails, we default to ok=True so a flaky verifier can't trap
    the agent in an endless revise loop.
    """
...
def verify_node(state: AgentState) -> dict:
    """Ask the model whether the execution result plausibly answers the question."""
...
def revise_node(state: AgentState) -> dict:
    """Fix the SQL using the verifier's complaint and the prior attempt."""
    result_text = state.execution.render() if state.execution else "no execution result"
    response = llm().invoke(
        [
...
def route_after_verify(state: AgentState) -> str:
    """Loop into revise unless the verifier is happy or we've hit the cap."""
    if state.verify_ok:
        return "end"
